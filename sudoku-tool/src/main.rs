...
use bf_solver::{find_all_solutions, find_one_solution};
use std::fs;
use std::fs::File;
use std::io::Write;
use std::path::Path;
...
fn main() {
    ...
    stats_data.push(
        "Puzzle,NodesExplored,Backtracks,MaxRecursionDepth,SearchDurationMicros,SolutionsFound"
            .to_string(),
    );
    ...
    for file_path in puzzle_files.iter() {
        ...
        match Sudoku::from_file(file_path) {
            ...
            Err(e) => {
                ...
                let csv_line = format!("{},ERROR,ERROR,ERROR,ERROR,ERROR", file_path);
            }
        }
    }
